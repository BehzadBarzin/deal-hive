...
import { ViewsByDealHiveChart } from "../_components/charts/ViewsByDealHiveChart";
...
export default async function AnalyticsPage({
  ...
  const searchParamFields = await searchParams;
  ...
  const interval =
    CHART_INTERVALS[
      searchParamFields.interval as keyof typeof CHART_INTERVALS
    ] ?? CHART_INTERVALS.last7Days;
  const timezone = searchParamFields.timezone || "UTC";
  const productId = searchParamFields.productId;
        ...
        <HasPermission permission={canAccessAnalytics}>
          <div className="flex gap-2">
            ...
            <ProductDropdown
              ...
              searchParams={searchParamFields}
            />
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="outline">
              ...
              <DropdownMenuContent>
                <DropdownMenuItem asChild>
                  <Link
                    href={createURL("/dashboard/analytics", searchParamFields, {
                      timezone: "UTC",
                    })}
                  >
                    UTC
                ...
                <TimezoneDropdownMenuItem searchParams={searchParamFields} />
              </DropdownMenuContent>
            </DropdownMenu>
          </div>
        </HasPermission>
...
// =================================================================================================
async function ViewsByDealHiveCard(
  props: Parameters<typeof getViewsByDealHiveChartData>[0]
) {
  const chartData = await getViewsByDealHiveChartData(props);

  return (
    <Card>
      <CardHeader>
        <CardTitle>Visitors Per DealHive Group</CardTitle>
      </CardHeader>
      <CardContent>
        <ViewsByDealHiveChart chartData={chartData} />
      </CardContent>
    </Card>
  );
}
